  ...
  if (isEditMode.value) {
    ...
    if (task) {
      ...
      if (!categories.value.some(c => c.name === task.category)) {
        categories.value.push({ name: task.category, icon: 'folder_open' })
      }
    } else {
      // 找不到任务，退回
      router.replace('/todo')
...
const showNewCategoryPrompt = () => {
  ...
  if (newCat && newCat.trim()) {
    ...
    if (!categories.value.some(c => c.name === trimmed)) {
      categories.value.push({ name: trimmed, icon: 'folder_open' })
    }
...
<template>
  <div class="w-full h-full overflow-y-auto bg-background text-on-background flex flex-col min-h-screen relative pb-32">
    ...
    <header class="mobile-safe-header w-full sticky top-0 z-50 bg-background/90 backdrop-blur-md flex items-center justify-between px-6 pb-4 border-b border-outline-variant/10">
      ...
      <button 
        v-if="isEditMode"
        @click="deleteTask"
        class="p-2 rounded-full hover:bg-red-50 text-red-600 hover:text-red-700 transition-colors active:scale-95 duration-200 flex items-center justify-center"
        title="删除任务"
      >
        <span class="material-symbols-outlined text-2xl">delete</span>
      </button>
      <div v-else class="w-10"></div> <!-- Placeholder to keep title centered -->
    </header>
    ...
    <main class="flex-grow px-6 pt-6 flex flex-col gap-6 max-w-lg mx-auto w-full">
      ...
      <section class="flex flex-col gap-3 animate-entrance" style="animation-delay: 0.08s;">
        ...
        <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          ...
          >
            <span class="material-symbols-outlined text-[16px] leading-none" :class="{ 'filled': selectedCategory === cat.name }">
              {{ cat.icon }}
            </span>
            <span>{{ cat.name }}</span>
          </button>
      ...
      <section class="flex flex-col gap-3 animate-entrance" style="animation-delay: 0.12s;">
        <div class="flex justify-between items-center">
          ...
          <div class="flex items-center gap-2">
            ...
            <label class="cursor-pointer flex items-center justify-center p-1.5 rounded-full hover:bg-surface-container-high text-primary">
    ...
    <div class="mobile-safe-bottom fixed bottom-0 left-0 w-full bg-gradient-to-t from-background via-background to-transparent pt-8 px-6 z-40">
      <div class="max-w-lg mx-auto w-full">
        <button 
          @click="saveTask"
          class="w-full bg-primary text-on-primary py-4 rounded-full font-label text-sm font-bold shadow-lg shadow-primary/20 hover:bg-primary/95 transition-all duration-200 active:scale-95"
        >
