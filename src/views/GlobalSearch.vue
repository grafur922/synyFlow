...
function sourceLabel(value: string) {
  return ({ terra: 'Terra', xiaomi: '小米', rss: 'RSS', blog: '博客', travel: '旅行', upload: '文档' } as Record<string, string>)[value] || value
}
...
<template>
  <div class="flex h-full min-h-0 flex-col bg-background">
    <header class="flex flex-shrink-0 flex-wrap items-center justify-between gap-3 border-b border-outline-variant/25 px-5 py-4 md:px-8">
      <div>
        <div class="flex items-center gap-2"><span class="material-symbols-outlined text-primary">manage_search</span><h2 class="font-headline text-2xl font-bold">全局搜索</h2></div>
      ...
      <div class="flex w-full items-center justify-end gap-2 sm:w-auto"><button v-if="status?.conflictCount" class="flex h-10 items-center gap-1.5 rounded-lg border border-outline-variant/40 px-3 text-sm font-bold" :class="status.unresolvedConflictCount ? 'text-error' : 'text-secondary'" @click="openConflictArchive"><span class="material-symbols-outlined text-[19px]">difference</span><span>冲突</span><span v-if="status.unresolvedConflictCount" class="rounded-full bg-error px-1.5 py-0.5 text-[9px] text-on-error">{{ status.unresolvedConflictCount }}</span></button><button class="flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-bold text-on-primary disabled:opacity-40" :disabled="syncing" @click="syncResources"><span class="material-symbols-outlined text-[20px]" :class="{ 'animate-spin': syncing }">sync</span>{{ syncing ? '同步索引中…' : '同步 Todo 与笔记' }}</button></div>
    </header>

    <div v-if="status?.unresolvedConflictCount" class="flex flex-shrink-0 items-center gap-2 border-b border-error/20 bg-error-container/45 px-5 py-2.5 text-xs text-on-error-container md:px-8"><span class="material-symbols-outlined text-[18px]">difference</span><span class="min-w-0 flex-1">有 {{ status.unresolvedConflictCount }} 条跨端版本冲突等待处理，当前索引版本保持不变。</span><button class="flex-shrink-0 font-bold underline" @click="openConflictArchive">查看冲突</button></div>
    <div v-if="status?.xiaomiSync?.state === 'failed'" class="flex flex-shrink-0 items-start gap-2 border-b border-outline-variant/20 bg-error-container/50 px-5 py-2.5 text-xs text-on-error-container md:px-8"><span class="material-symbols-outlined text-[18px]">sync_problem</span><span>小米笔记索引未更新：{{ status.xiaomiSync.error }}</span></div>
    ...
    <div class="flex-shrink-0 border-b border-outline-variant/20 bg-surface-container-low/40 p-4 md:px-8">
      <div class="mx-auto grid max-w-5xl grid-cols-2 gap-2 lg:grid-cols-6">
        <label class="col-span-2 flex items-center gap-2 rounded-xl border border-outline-variant/30 bg-surface-bright px-4 py-3 focus-within:ring-2 focus-within:ring-primary/25 lg:col-span-4"><span class="material-symbols-outlined text-secondary">search</span><input v-model="query" autofocus class="w-full border-0 bg-transparent p-0 focus:ring-0" placeholder="搜索笔记正文、Todo、标签……" /></label>
        ...
        <label class="filter-field"><span class="material-symbols-outlined">sell</span><input v-model="tag" aria-label="标签筛选" maxlength="120" placeholder="标签" /></label>
        <label class="filter-field"><span class="material-symbols-outlined">folder_data</span><input v-model="project" aria-label="项目筛选" maxlength="120" placeholder="项目" /></label>
        <label class="filter-field"><span class="material-symbols-outlined">location_on</span><input v-model="location" aria-label="位置筛选" maxlength="120" placeholder="位置" /></label>
    ...
    <div class="min-h-0 flex-1 overflow-y-auto p-5 md:p-8">
      <div class="mx-auto max-w-5xl">
        <div v-if="error" class="mb-4 flex items-start gap-2 rounded-xl bg-error-container/60 p-3 text-sm text-on-error-container"><span class="material-symbols-outlined">error</span>{{ error }}</div>
        <div v-if="loading" class="flex min-h-60 items-center justify-center text-secondary"><span class="material-symbols-outlined mr-2 animate-spin">progress_activity</span>搜索中…</div>
        <div v-else-if="!results.length" class="flex min-h-72 flex-col items-center justify-center text-center text-secondary"><span class="material-symbols-outlined mb-3 text-5xl text-outline">travel_explore</span><h3 class="font-headline text-xl font-bold text-on-surface">{{ emptyMessage }}</h3><p v-if="!status?.resourceCount" class="mt-2 text-sm">首次使用请点击“同步 Todo 与笔记”建立加密索引。</p></div>
        <div v-else class="space-y-3">
          <button v-for="result in results" :key="result.id" class="w-full rounded-xl border border-outline-variant/25 bg-surface-bright p-5 text-left shadow-sm transition hover:border-primary/30 hover:bg-surface-container-low" @click="openDetail(result)">
            ...
            <div class="mt-3 flex flex-wrap items-center gap-2"><span v-for="itemTag in result.tags" :key="itemTag" class="rounded-full bg-secondary-container px-2 py-0.5 text-[10px] font-bold text-on-secondary-container">#{{ itemTag }}</span><span v-for="itemProject in result.context.projects.slice(0, 2)" :key="`project-${itemProject}`" class="flex items-center gap-1 text-[10px] font-bold text-secondary"><span class="material-symbols-outlined text-[13px]">folder_data</span>{{ itemProject }}</span><span v-if="result.context.locations[0]" class="flex items-center gap-1 text-[10px] font-bold text-secondary"><span class="material-symbols-outlined text-[13px]">location_on</span>{{ result.context.locations[0].name }}</span><time class="ml-auto text-[10px] text-secondary">{{ formatDate(result.updatedAt) }}</time></div>
          </button>
        </div>
      </div>
    </div>
      ...
      <div v-if="conflictOpen" class="fixed inset-0 z-[95] flex justify-end bg-black/25" @click.self="conflictOpen = false">
        <aside role="dialog" aria-modal="true" aria-labelledby="resource-conflicts-title" class="flex h-full w-full max-w-6xl flex-col border-l border-outline-variant/30 bg-background shadow-2xl">
          <header class="flex flex-wrap items-start justify-between gap-3 border-b border-outline-variant/25 px-4 py-3 md:px-5">
            ...
            <button class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg text-secondary hover:bg-surface-container-high" aria-label="关闭冲突归档" @click="conflictOpen = false"><span class="material-symbols-outlined">close</span></button>
          </header>
          ...
          <div v-if="conflictError" class="flex items-start gap-2 border-b border-error/20 bg-error-container/55 px-4 py-2.5 text-xs text-on-error-container md:px-5"><span class="material-symbols-outlined text-[18px]">error</span><span class="min-w-0 flex-1">{{ conflictError }}</span><button class="flex-shrink-0" aria-label="关闭冲突错误" @click="conflictError = ''"><span class="material-symbols-outlined text-[17px]">close</span></button></div>
          <div class="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[320px_minmax(0,1fr)]">
            <section class="min-h-0 overflow-y-auto border-r border-outline-variant/25 bg-surface-container-low/45 p-3" :class="{ 'hidden md:block': selectedConflict }">
              <div v-if="conflictLoading && !conflicts.length" class="flex min-h-48 items-center justify-center text-sm text-secondary"><span class="material-symbols-outlined mr-2 animate-spin">progress_activity</span>读取冲突…</div>
              ...
              <div v-if="!conflictLoading && !conflicts.length" class="flex min-h-48 flex-col items-center justify-center text-center text-secondary"><span class="material-symbols-outlined mb-2 text-4xl">difference</span><p class="text-sm font-bold">当前列表为空</p></div>
            </section>

            <section class="min-h-0" :class="selectedConflict ? 'block' : 'hidden md:block'">
              <div v-if="conflictLoading && !selectedConflict" class="flex h-full items-center justify-center text-secondary"><span class="material-symbols-outlined mr-2 animate-spin">progress_activity</span>读取版本…</div>
              <div v-else-if="selectedConflict" class="flex h-full min-h-0 flex-col">
                <div class="flex items-center gap-2 border-b border-outline-variant/20 px-3 py-2 md:px-4"><button class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg text-secondary md:hidden" aria-label="返回冲突列表" @click="selectedConflict = undefined"><span class="material-symbols-outlined">arrow_back</span></button><div class="min-w-0 flex-1"><p class="truncate text-sm font-bold">{{ conflictReasonLabel(selectedConflict.reason) }}</p><p class="mt-0.5 text-[10px] text-secondary">{{ sourceLabel(selectedConflict.source) }} · {{ formatDate(selectedConflict.detectedAt) }}</p></div><span class="flex-shrink-0 text-xs font-bold" :class="selectedConflict.status === 'unresolved' ? 'text-error' : 'text-primary'">{{ conflictResolutionLabel(selectedConflict.resolution) }}</span></div>
              ...
              <div v-else class="flex h-full flex-col items-center justify-center text-center text-secondary"><span class="material-symbols-outlined mb-2 text-5xl">difference</span><p class="text-sm font-bold">选择一条冲突比较版本</p></div>
            </section>
          </div>
        </aside>
      </div>
    ...
    <Teleport to="body">
      <div v-if="selected || detailLoading" class="fixed inset-0 z-[90] flex justify-end bg-black/20 backdrop-blur-[2px]" @click.self="selected = undefined">
        <aside class="flex h-full w-full max-w-2xl flex-col border-l border-outline-variant/30 bg-background shadow-2xl">
          <div v-if="detailLoading" class="flex flex-1 items-center justify-center text-secondary"><span class="material-symbols-outlined mr-2 animate-spin">progress_activity</span>读取资源…</div>
          <template v-else-if="selected"><header class="flex items-start justify-between gap-3 border-b border-outline-variant/25 p-5"><div><div class="flex items-center gap-2 text-xs font-bold text-secondary"><span>{{ typeLabel(selected.type) }}</span><span>·</span><span>{{ sourceLabel(selected.source) }}</span><span>·</span><span>{{ privacyLabel(selected.privacy) }}</span></div><h3 class="mt-2 font-headline text-2xl font-bold">{{ selected.title }}</h3></div><button class="flex h-9 w-9 items-center justify-center rounded-lg text-secondary hover:bg-surface-container-high" @click="selected = undefined"><span class="material-symbols-outlined">close</span></button></header><div class="min-h-0 flex-1 overflow-y-auto p-5"><p class="text-sm leading-6 text-secondary">{{ selected.summary }}</p><div v-if="selected.context.projects.length || selected.context.locations.length || contextTimeLabel(selected)" class="mt-4 flex flex-wrap gap-x-4 gap-y-2 border-y border-outline-variant/20 py-3 text-xs text-secondary"><span v-if="selected.context.projects.length" class="flex items-center gap-1"><span class="material-symbols-outlined text-[16px]">folder_data</span>{{ selected.context.projects.join('、') }}</span><span v-if="contextTimeLabel(selected)" class="flex items-center gap-1"><span class="material-symbols-outlined text-[16px]">schedule</span>{{ contextTimeLabel(selected) }}</span><span v-if="selected.context.locations.length" class="flex items-center gap-1"><span class="material-symbols-outlined text-[16px]">location_on</span>{{ selected.context.locations.map((item) => item.name).join('、') }}</span></div><pre class="mt-5 whitespace-pre-wrap rounded-xl bg-surface-container-low p-5 font-body text-sm leading-7 text-on-surface">{{ selected.content || '该资源没有可显示的正文' }}</pre></div><footer class="flex items-center justify-between border-t border-outline-variant/25 p-4"><span class="text-xs text-secondary">索引于 {{ formatDate(selected.indexedAt) }}</span><button v-if="canOpenSource(selected)" class="rounded-xl bg-primary px-4 py-2 text-sm font-bold text-on-primary" @click="openSource(selected)">打开来源</button></footer></template>
        </aside>
      </div>
    </Teleport>
  </div>
...
.filter-field > .material-symbols-outlined { @apply flex-shrink-0 text-[17px]; }
